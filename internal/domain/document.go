...
type DocumentChunk struct {
	...
	Embedding  []float32 `gorm:"-" json:"-"` // pgvector integration (ignored by GORM)
...
type DocumentUsecase interface {
	...
	Delete(ctx context.Context, tenantID, docID uuid.UUID) error
	UpdateText(ctx context.Context, tenantID, userID, docID uuid.UUID, text string, title string, status string) error
	GetRaw(ctx context.Context, tenantID, docID uuid.UUID) (text string, hash string, err error)
}
