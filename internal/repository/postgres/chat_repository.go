...
func (r *chatRepository) LogTokenUsage(ctx context.Context, tenantID uuid.UUID, model string, promptTokens, completionTokens int) error {
	cost := float64(promptTokens+completionTokens) * 0.00000015
	err := r.db.WithContext(ctx).Exec(
		"INSERT INTO token_usage_ledgers (id, tenant_id, model, prompt_tokens, completion_tokens, cost, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())",
		uuid.New(), tenantID, model, promptTokens, completionTokens, cost,
	).Error
	if err != nil {
