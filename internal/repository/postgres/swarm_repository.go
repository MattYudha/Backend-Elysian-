...
func (r *SwarmRepository) ListByTenant(ctx context.Context, tenantID string, documentID string, limit, offset int) ([]*domain.SwarmTask, int64, error) {
	...
	if documentID != "" {
		queryCount = queryCount.Where("swarm_tasks.document_id = ?", documentID)
	}
	...
	queryFind := r.db.WithContext(ctx).
		Table("swarm_tasks").
		Select("swarm_tasks.*").
		Joins("JOIN documents ON documents.id = swarm_tasks.document_id").
		Where("documents.tenant_id = ?", tenantID)
	...
	if documentID != "" {
		queryFind = queryFind.Where("swarm_tasks.document_id = ?", documentID)
	}
