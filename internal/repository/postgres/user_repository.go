...
// GetProfile retrieves the extended user profile (bio, links).
// If no profile exists yet, it returns a default empty profile (does NOT persist it).
func (r *UserRepository) GetProfile(ctx context.Context, userID string) (*domain.UserProfile, error) {
	...
	err = r.db.WithContext(ctx).Where("user_id = ?", uid).First(&profile).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		// Return a default empty profile — not persisted until first save
...
// UpdateProfile upserts the user's extended profile (bio, links).
func (r *UserRepository) UpdateProfile(ctx context.Context, profile *domain.UserProfile) error {
	result := r.db.WithContext(ctx).
		Where("user_id = ?", profile.UserID).
		Assign(domain.UserProfile{Bio: profile.Bio, LinksJSON: profile.LinksJSON}).
		FirstOrCreate(profile)
	if result.Error != nil {
		return fmt.Errorf("failed to upsert user profile: %w", result.Error)
	}
	// If row already existed (FirstOrCreate found it), update it explicitly
	if result.RowsAffected == 0 {
