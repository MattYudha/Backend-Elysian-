...
type MockDocumentUsecase struct {
	...
	ListDocumentsFunc func(ctx context.Context, tenantID uuid.UUID, limit, offset int) ([]*domain.Document, int64, error)
	...
	UpdateTextFunc    func(ctx context.Context, tenantID, userID, docID uuid.UUID, text string, title string, status string) error
	GetRawFunc        func(ctx context.Context, tenantID, docID uuid.UUID) (string, string, error)
}
...
func (m *MockDocumentUsecase) ListDocuments(ctx context.Context, tenantID uuid.UUID, limit, offset int) ([]*domain.Document, int64, error) {
...
func (m *MockDocumentUsecase) UpdateText(ctx context.Context, tenantID, userID, docID uuid.UUID, text string, title string, status string) error {
	if m.UpdateTextFunc != nil {
		return m.UpdateTextFunc(ctx, tenantID, userID, docID, text, title, status)
	}
	return nil
}

func (m *MockDocumentUsecase) GetRaw(ctx context.Context, tenantID, docID uuid.UUID) (string, string, error) {
	if m.GetRawFunc != nil {
		return m.GetRawFunc(ctx, tenantID, docID)
	}
	return "", "", nil
}
...
func TestDocumentHandler_Approve(t *testing.T) {
	...
	t.Run("Approve Error Response", func(t *testing.T) {
		...
		if resp["error"] != "An internal server error occurred" {
