...
func (h *SwarmHandler) List(c *gin.Context) {
	...
	tasks, total, err := h.swarmUsecase.ListSwarmTasks(c.Request.Context(), tenantID, documentID, limit, offset)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
type SaveOverrideRequest struct {
	ItemName        string `json:"item_name" binding:"required"`
	OriginalVerdict string `json:"original_verdict" binding:"required"`
	NewVerdict      string `json:"new_verdict" binding:"required"`
	Justification   string `json:"justification" binding:"required"`
}

func (h *SwarmHandler) SaveOverride(c *gin.Context) {
	var req SaveOverrideRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid request payload"})
		return
	}
	...
	override := &domain.AuditorOverride{
		ID:              uuid.New().String(),
		TenantID:        tenantIDStr,
		ItemName:        req.ItemName,
		OriginalVerdict: req.OriginalVerdict,
		NewVerdict:      req.NewVerdict,
		Justification:   req.Justification,
		UserID:          userIDStr,
	}

	if err := h.swarmUsecase.SaveOverride(c.Request.Context(), override); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{
		"status":  "success",
		"message": "Auditor override saved successfully",
		"data":    override,
	})
}
