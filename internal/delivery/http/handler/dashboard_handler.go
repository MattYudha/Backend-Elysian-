...
func (h *DashboardHandler) GetRegionDetails(c *gin.Context) {
	tenantID := middleware.MustGetTenantIDFromContext(c)
	region := c.Query("region")
	if region == "" {
		c.JSON(http.StatusBadRequest, ErrorResponse{Error: "region query parameter is required"})
		return
	}

	details, err := h.useCase.GetRegionDetails(c.Request.Context(), tenantID, region)
	if err != nil {
		c.JSON(http.StatusInternalServerError, ErrorResponse{Error: err.Error()})
		return
	}
