...
func SetupRoutes(
	...
	nemesisHandler *handler.NemesisHandler,
	guardrailHandler *handler.GuardrailHandler,
	actionCenterHandler *handler.ActionCenterHandler,
	...
	// Root-level mock upload to bypass PWA service worker path matching (/api/...)
	router.PUT("/mock-upload", documentHandler.MockUpload)
	...
	api := router.Group("/api")
	{
		...
		v1 := api.Group("/v1")
		{
			...
			v1.PUT("/documents/mock-upload", documentHandler.MockUpload)
			...
			docs.Use(authMiddleware, middleware.TenantMiddleware())
			{
				...
				docs.GET("/:id/raw", documentHandler.GetRaw)
			}
			...
			swarm := v1.Group("/swarm")
			{
				...
				protectedSwarm.Use(authMiddleware, middleware.TenantMiddleware())
				{
					...
					protectedSwarm.POST("/override", swarmHandler.SaveOverride)
				}
			}
			...
			// Nemesis Query (Strict Multi-Tenancy Enforced)
			nemesis := v1.Group("/nemesis")
			nemesis.Use(authMiddleware, middleware.TenantMiddleware())
			{
				nemesis.GET("/query", nemesisHandler.Query)
			}

			// Action Center (Strict Multi-Tenancy Enforced)
			actionCenter := v1.Group("/action-center")
			actionCenter.Use(authMiddleware, middleware.TenantMiddleware())
			{
				actionCenter.GET("/items", actionCenterHandler.List)
				actionCenter.POST("/items/:id/resolve", actionCenterHandler.Resolve)
			}

			// Guardrails (Strict Multi-Tenancy Enforced)
			guardrails := v1.Group("/guardrails")
			guardrails.Use(authMiddleware, middleware.TenantMiddleware())
			{
				guardrails.POST("/precheck", guardrailHandler.Precheck)
			}
			...
			dashboard.Use(authMiddleware, middleware.TenantMiddleware())
			{
				...
				dashboard.GET("/regions/details", dashboardHandler.GetRegionDetails)
			}
